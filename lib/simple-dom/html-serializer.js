...
HTMLSerializer.prototype.escapeAttrValue = function(attrValue) {
  if (TEST_ATTR_REGEX.test(attrValue)) {
    return attrValue.replace(ATTR_REGEX, matcher);
  }
...
HTMLSerializer.prototype.escapeText = function(textNodeValue) {
  if (TEST_TEXT_REGEX.test(textNodeValue)) {
    return textNodeValue.replace(TEXT_REGEX, matcher);
  }
